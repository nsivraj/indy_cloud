...
4) docker run -v /Users/norm/forge/work/code/indy_cloud:/root/indy_cloud -p 9000:9000 -p 9001:9001 -it ${USER}/indy_cloud:version1 /bin/bash OR docker run -v /root/forge/personal/code/indy_cloud:/root/indy_cloud -p 80:80 -p 81:81 -it ${USER}/indy_cloud /bin/bash
5) Once inside the container then do:
    a) cd /root/indy_cloud/java
    b) ./run.sh
6) docker ps
7) docker commit [CONTAINER ID] norm/indy_cloud:version1 -- where [CONTAINER ID] comes from 'docker ps' and [version_N] is version1, version2, ... etc.
